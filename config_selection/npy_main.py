...
from config_selection.npy_feature_parser import NpyFeatureParser
...
def _construct_X_and_feed_dict(features_read):
    ...
    label = tf.placeholder(tf.int64, features_read['label'].shape)
    model['label'] = label
    
    # Same circus for forloeb
    forloeb = tf.placeholder(tf.int64, features_read['forloeb'].shape)
    model['forloeb'] = forloeb
    
    # Fill actual values into seq_len, label, forloeb
    to_feed[seq_len] = features_read['seq_len']
    to_feed[label] = features_read['label']
    to_feed[forloeb] = features_read['forloeb']
    return X, X_reordered, model, to_feed
...
class ConfigSelection(object):
    ...
    def _read_next_input(self):
        ...
        with graph.as_default():
            *_, input_model, input_feed_dict = _construct_X_and_feed_dict(self.features_read)
            ...
            self.input_model = sirs_classifier.create_model(None, global_step, input_model, 
                                                            False, INPUT_SIZE, CONTEXT_SIZE, NUM_CLASSES,
                                                            USE_TEXT, use_char_autoencoder=USE_AUTOENCODER,
            ...
            with tf.Session() as s:
                ...
                # Read the next input
                self.input_selection, y_hat = s.run([self.input_model['offsets'],
        ...
        logger.info("Read sample {} with label \n{}".format(self.parser.samples_read(),
                                                            self.features_read['label']))
    ...
    def _test_configuration(self, config):
        ...
        with graph.as_default():
            ...
            X, X_reordered, self.features_batch, feed_dict = _construct_X_and_feed_dict(self.features_read)
