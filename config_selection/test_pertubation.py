...
class PertubationTest(unittest.TestCase):
    def runTest(self):
        with tf.Graph().as_default():
            ...
            R_values = [0.2, 0.3]
            R = tf.SparseTensor(R_indices, R_values, (1, 4, 5))

            def _scale_sum(x):
                x = tf.sparse_reshape(x, (4, 5))
                ...
                # reduce sum
                res = tf.squeeze(tf.reduce_sum(mult, 0))
                return {'y_hat': res}
